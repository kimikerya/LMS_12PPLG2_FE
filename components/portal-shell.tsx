...
export function PortalShell({ identity, fullName, children }: { identity: Identity; fullName?: string; children: React.ReactNode }) {
  ...
  const title = learningTitle ?? (pathname === "/pengaturan" ? "Pengaturan" : items.find(i => active(i.href))?.label ?? "Portal Akademik");
  return <div className="portal">
    ...
    <aside id="sidebar" className={`sidebar ${open ? "is-open" : ""}`}>
      ...
      <div className="sidebar-bottom">{identity.role !== "student" && <Link href="/pengaturan" className={`nav-item ${pathname === "/pengaturan" ? "active" : ""}`} onClick={() => setOpen(false)}><Icon name="settings" />Pengaturan</Link>}<div className="account"><span className="avatar">{accountName.split(/\s+/).slice(0,2).map(n=>n[0]).join("").toUpperCase()}</span><span><strong>{accountName}</strong><small>{roleLabels[identity.role]}</small></span><form action={logout}><button title="Keluar dari akun" aria-label="Keluar dari akun" className="icon-button"><Icon name="logout" /></button></form></div></div>
    </aside>
