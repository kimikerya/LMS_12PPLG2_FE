...
export async function POST(request: Request, { params }: { params: Promise<{ operation: string }> }) {
  ...
  const id = new URL(request.url).searchParams.get("assignment");
  const creating = operation === "create"; const editing = operation === "edit";
  if ((!creating && !editing && operation !== "submit") || (!creating && !/^[1-9]\d*$/.test(id ?? ""))) return Response.json({ error: "Tugas tidak valid." }, { status: 400 });
  if (session.identity.role !== (creating || editing ? "teacher" : "student")) return Response.json({ error: "Akses tidak diizinkan." }, { status: 403 });
  ...
  try {
    const response = await fetch(new URL(creating ? "/api/assignments/upload" : editing ? `/api/assignments/${id}/upload` : `/api/assignments/${id}/submissions/upload`, process.env.BACKEND_URL ?? "http://127.0.0.1:8080"), {
    ...
    if (response.ok) { revalidatePath("/tugas", "layout"); revalidatePath("/kelas", "layout"); revalidatePath("/dashboard"); }
    return Response.json(result, { status: response.status });
  } catch {
    return Response.json({ error: bytes > 27 * 1024 * 1024 ? "Total unggahan maksimal 25 MB." : "Unggahan belum dapat dikonfirmasi. Periksa koneksi dan daftar tugas sebelum mencoba lagi." }, { status: 503 });
  }
}
