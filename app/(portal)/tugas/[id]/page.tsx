...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{kelas?:string;mapel?:string}>}) {if(["curriculum","principal"].includes((await requireSession()).identity.role))return <MonitoringLearningDetail kind="assignments" id={(await params).id}/>;if((await requireSession()).identity.role==="teacher")return <TeacherLearningDetail kind="assignments" id={(await params).id}/>;return <StudentLearningDetail kind="assignments" id={(await params).id} context={(await searchParams).kelas} subjectContext={(await searchParams).mapel}/>;}
