import { TeacherClasses } from "@/modules/teacher/classes";
import { MonitoringClasses } from "@/modules/monitoring/classes";
import { monitoringClasses } from "@/modules/monitoring/data";
import { teacherClasses } from "@/modules/teacher/data";
...
export default async function ClassesPage({searchParams}:{searchParams:Promise<{deleted?:string;kelas?:string}>}) {
  ...
  return <><div className="page-heading"><div><span className="eyebrow">RUANG BELAJAR</span><h1>Kelas</h1><p>Daftar kelas sesuai keanggotaan dan akses akun Anda.</p></div><div className="button-row"><span className="count-pill">{data.length} kelas</span></div></div><section className="panel"><ClassList items={data}  /></section></>;
}
